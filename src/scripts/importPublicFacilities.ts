...
const FACILITY_TYPE_MAPPING: Record<string, string> = {
  // 농구 (sport_type: "농구")
  농구장: "농구",
  체육관: "농구", // 복합시설은 농구로 매핑
 ...
 */
function getFacilityTypeName(facilityType: string): string | null {
  if (!facilityType) return null;

  // 제외 목록 확인
  for (const excluded of EXCLUDED_TYPES) {
    if (facilityType.includes(excluded)) return null;
  }
 ...
 */
async function findRegion(city: string, district: string) {
...
function readExcelFile(filePath: string): any[] {
  ...
  // 첫 번째 줄이 비어있을 경우 제거
  if (data.length > 0 && data[0].every((cell: any) => !cell)) {
    data.shift();
  }
 ...
 * 메인 임포트 함수
 */
async function importPublicFacilities(filePath: string, operatorId: string) {
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        const address =
          row["소재지도로명주소"]?.trim() || row["소재지지번주소"]?.trim();
        ...
        if (!facilityName || !address) {
          skipCount++;
          console.log(
            `[${i}] 필수필드 누락: 시설명="${facilityName || "EMPTY"}" 주소="${address || "EMPTY"}"`,
          );
          continue;
        }

        console.log(
          `[${i}] 시설명="${facilityName}" | 개방시설명="${facilityTypeDescription}" | 개방시설유형구분="${facilityTypeColumn}"`,
        );
        ...
        console.log(
          `[${i}] 주소 파싱: ${address} → ${parsedAddress.city} / ${parsedAddress.district}`,
        );
        ...
        if (!region) {
          ...
          skipCount++;
          console.log(
            `[${i}] 지역 미등록: ${parsedAddress.city} / ${parsedAddress.district}`,
          );
          continue;
        }

        console.log(
          `[${i}] ✓ 임포트 가능: ${facilityName} / ${sportTypeName} / ${region.district}`,
        );
    ...
    console.log(
      `   제외: ${skipCount - regionNotFoundCount}개 (시설유형 제외)`,
    );
...
if (!filePath || !operatorId) {
  console.error(
    "사용법: node dist/scripts/importPublicFacilities.js <엑셀파일경로> <operator_id>",
  );
  console.error(
    "예시: node dist/scripts/importPublicFacilities.js ./data.xls 1",
  );
  process.exit(1);
}
